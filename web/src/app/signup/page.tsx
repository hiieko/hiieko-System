...
export default function SignupPage() {
  ...
  if (success) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-md bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
        ...
        <Link href="/login" className="inline-block px-6 py-2 bg-hii-500 hover:bg-hii-600 text-white font-semibold text-sm rounded-lg">
  ...
  const inp = 'w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-hii-500/30 focus:outline-none';
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-14 h-14 bg-hii-500 rounded-2xl shadow-lg mb-3"><span className="text-white font-extrabold text-lg">H</span></div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.acceptTerms} onChange={e => setForm({...form, acceptTerms: e.target.checked})} className="w-4 h-4 text-hii-500 rounded" />
            <span className="text-xs text-slate-600">{t('application.accept_terms')}</span>
          </label>
          <button type="submit" disabled={loading} className="w-full py-3 bg-hii-500 hover:bg-hii-600 disabled:opacity-50 text-white font-bold text-sm rounded-lg">{loading ? '...' : t('application.submit')}</button>
          <p className="text-center text-sm text-slate-500"><Link href="/login" className="font-semibold text-hii-600">{t('auth.has_account')}</Link></p>
        </form>
      </div>
    </div>
  );
