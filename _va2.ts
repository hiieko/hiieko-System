...
async function main(): Promise<number> {
  ...
  try {
    ...
    const fkResults = await Promise.all(ids.map((sql) => pool.query(sql)));
...
// 4. Business invariants
    ...
    if ((legacyCount.rows[0]?.n ?? 0) > 0) {
      ...
      for (const [target, legacyTable] of pairs) {
        const t = await pool.query(`SELECT count(*)::int n FROM public.${target}`);
        const l = await pool.query(`SELECT count(*)::int n FROM legacy.${legacyTable}`);
        results.push({
